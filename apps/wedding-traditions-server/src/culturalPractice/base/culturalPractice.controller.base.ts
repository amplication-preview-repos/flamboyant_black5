/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import * as nestAccessControl from "nest-access-control";
import * as defaultAuthGuard from "../../auth/defaultAuth.guard";
import { CulturalPracticeService } from "../culturalPractice.service";
import { AclValidateRequestInterceptor } from "../../interceptors/aclValidateRequest.interceptor";
import { AclFilterResponseInterceptor } from "../../interceptors/aclFilterResponse.interceptor";
import { CulturalPracticeCreateInput } from "./CulturalPracticeCreateInput";
import { CulturalPractice } from "./CulturalPractice";
import { CulturalPracticeFindManyArgs } from "./CulturalPracticeFindManyArgs";
import { CulturalPracticeWhereUniqueInput } from "./CulturalPracticeWhereUniqueInput";
import { CulturalPracticeUpdateInput } from "./CulturalPracticeUpdateInput";

@swagger.ApiBearerAuth()
@common.UseGuards(defaultAuthGuard.DefaultAuthGuard, nestAccessControl.ACGuard)
export class CulturalPracticeControllerBase {
  constructor(
    protected readonly service: CulturalPracticeService,
    protected readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}
  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Post()
  @swagger.ApiCreatedResponse({ type: CulturalPractice })
  @nestAccessControl.UseRoles({
    resource: "CulturalPractice",
    action: "create",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async createCulturalPractice(
    @common.Body() data: CulturalPracticeCreateInput
  ): Promise<CulturalPractice> {
    return await this.service.createCulturalPractice({
      data: {
        ...data,

        wedding: data.wedding
          ? {
              connect: data.wedding,
            }
          : undefined,
      },
      select: {
        createdAt: true,
        description: true,
        id: true,
        name: true,
        updatedAt: true,

        wedding: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get()
  @swagger.ApiOkResponse({ type: [CulturalPractice] })
  @ApiNestedQuery(CulturalPracticeFindManyArgs)
  @nestAccessControl.UseRoles({
    resource: "CulturalPractice",
    action: "read",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async culturalPractices(
    @common.Req() request: Request
  ): Promise<CulturalPractice[]> {
    const args = plainToClass(CulturalPracticeFindManyArgs, request.query);
    return this.service.culturalPractices({
      ...args,
      select: {
        createdAt: true,
        description: true,
        id: true,
        name: true,
        updatedAt: true,

        wedding: {
          select: {
            id: true,
          },
        },
      },
    });
  }

  @common.UseInterceptors(AclFilterResponseInterceptor)
  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: CulturalPractice })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CulturalPractice",
    action: "read",
    possession: "own",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async culturalPractice(
    @common.Param() params: CulturalPracticeWhereUniqueInput
  ): Promise<CulturalPractice | null> {
    const result = await this.service.culturalPractice({
      where: params,
      select: {
        createdAt: true,
        description: true,
        id: true,
        name: true,
        updatedAt: true,

        wedding: {
          select: {
            id: true,
          },
        },
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.UseInterceptors(AclValidateRequestInterceptor)
  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: CulturalPractice })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CulturalPractice",
    action: "update",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async updateCulturalPractice(
    @common.Param() params: CulturalPracticeWhereUniqueInput,
    @common.Body() data: CulturalPracticeUpdateInput
  ): Promise<CulturalPractice | null> {
    try {
      return await this.service.updateCulturalPractice({
        where: params,
        data: {
          ...data,

          wedding: data.wedding
            ? {
                connect: data.wedding,
              }
            : undefined,
        },
        select: {
          createdAt: true,
          description: true,
          id: true,
          name: true,
          updatedAt: true,

          wedding: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: CulturalPractice })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @nestAccessControl.UseRoles({
    resource: "CulturalPractice",
    action: "delete",
    possession: "any",
  })
  @swagger.ApiForbiddenResponse({
    type: errors.ForbiddenException,
  })
  async deleteCulturalPractice(
    @common.Param() params: CulturalPracticeWhereUniqueInput
  ): Promise<CulturalPractice | null> {
    try {
      return await this.service.deleteCulturalPractice({
        where: params,
        select: {
          createdAt: true,
          description: true,
          id: true,
          name: true,
          updatedAt: true,

          wedding: {
            select: {
              id: true,
            },
          },
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
